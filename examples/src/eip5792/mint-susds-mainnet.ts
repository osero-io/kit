...
import { sendWith, supportsAtomicBatch } from '@osero/client/eip5792';
...
async function main() {
  ...
  banner(`Prepare atomic USDC → sUSDS — ${chain.name}`);
  ...
  console.log(`  execution: ${prepared.value.plan.requirements.execution}`);
  ...
  if (!(await supportsAtomicBatch(wallet))) {
    console.error('wallet does not report EIP-5792 atomic batch support');
    process.exitCode = 1;
    return;
  }

  const result = await sendWith(wallet, prepared.value.plan, { fallbackToSequential: false });
  if (result.isErr()) {
