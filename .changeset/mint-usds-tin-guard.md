...
Failed guards now surface as a dedicated `SlippageError`, which extends `UnexpectedError` for compatibility while exposing the live output and guarded minimum. Because `sellGem` has no on-chain `minOut`, same-block fee changes after the preflight read remain possible and require a wrapper contract to eliminate completely.
